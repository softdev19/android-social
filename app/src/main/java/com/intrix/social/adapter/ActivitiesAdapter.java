 ...
 */
public class ActivitiesAdapter extends ArrayAdapter<Connection> {
    ...
    public ActivitiesAdapter(Context context, ArrayList<Connection> connections, ArrayList<Customer> customers) {
        super(context, R.layout.item_my_people, connections);
        this.customers = customers;
        mContext = context;
        localDate = new Date();
        myId = MainApplication.data.user.getId();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(conn.getStatus() != null && conn.getStatus().equalsIgnoreCase("connected"))
            viewHolder.name.setText(user.getName() +" is connected ");
        else {
            if (conn.getCustomer_id() == myId)
                viewHolder.name.setText("You sent request to " + user.getName());
            else
                viewHolder.name.setText(user.getName() + " sent you connect request");
        }
